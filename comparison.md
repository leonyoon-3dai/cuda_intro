...
| 관점 | 데모1 (순수 Python) | 데모2 (a) NumPy 벡터화 | 데모2 (b) multiprocessing | CUDA `<<<numBlocks,256>>>` (블로그) |
|---|---|---|---|---|
| 어디서 도는가 | 인터프리터 1개 | C 라이브러리 + SIMD 1코어 | 4개의 OS 프로세스, 각자 NumPy | NVIDIA GPU 의 수많은 스레드 |
| 병렬화 단위 | 없음 | SIMD 레인 (≈ 8 floats / 명령) | OS 프로세스 (=4) | warp(32 threads) × 수천 |
| 동기화 방법 | 없음 | 함수 반환 = 끝 | `Process.join()` | `cudaDeviceSynchronize()` |
| 메모리 모델 | Python list | NumPy ndarray (연속 메모리) | `multiprocessing.shared_memory` (POSIX shm) | Unified Memory + 페이지 폴트/프리페치 |
| 스레드 생성 비용 | 0 | 0 | 매우 큼 (수십~수백 ms, macOS spawn) | 매우 작음 (µs) |
| grid-stride loop | ❌ | (불필요, 한 번에 통째 처리) | ✅ `y[tid::stride] = ...` | ✅ `for i = idx; i < N; i += stride` |
| Mac 에서 직접 실행 | ✅ | ✅ | ✅ | ❌ (참고용 코드만) |
| N=1M 실측 (이 Mac) | 85.9 ms | **0.78 ms** | 374 ms | (T4 기준) ≈ 47 µs |
...
### 4.1 본 사용자 Mac (Intel x86_64, 4 cores, Python 3 + NumPy 2.4)
...
> 모든 값은 같은 측정 스크립트로 얻은 값이며, 각 모드의 *가장 빠른* 시간을 채택했습니다.
...
* **데모2(b)** 가 우리 환경에서 항상 (a) 보다 느린 이유는 명확히 *프로세스 spawn /
  shared memory 셋업 비용* 때문입니다. macOS Python 의 spawn 모드가 특히 느립니다.
  이 비용은 GPU 의 kernel launch 비용 (수 µs) 과 비교되지 않을 정도로 큽니다.
